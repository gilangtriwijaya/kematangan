...
<div class="container py-4">
    <h4 class="mb-4">Tambah Tingkat Penilaian</h4>
    ...
    <form action="{{ route('tingkat.store') }}" method="POST" class="card p-4 shadow-sm">
        @csrf
       ...
       <div class="mb-3">
            ...
            <select name="variabel_id" id="variabel_id" class="form-select" required>
                ...
                @foreach (\App\Models\VariabelPenilaian::where('kegiatan_id', $kegiatanAktif->id)->get() as $v)
                    <option value="{{ $v->id }}" {{ old('variabel_id') == $v->id ? 'selected' : '' }}>
                        {{ $v->kode }} - {{ $v->nama }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="label" id="label" class="form-control" value="{{ old('label') }}" placeholder="Contoh: I / II / III / A / B" required>
        </div>

        <div class="mb-3">
            <label for="poin" class="form-label">Poin</label>
            <input type="number" name="poin" id="poin" class="form-control" value="{{ old('poin') }}" placeholder="Contoh: 1 / 20 / 100" required min="0">
        </div>
